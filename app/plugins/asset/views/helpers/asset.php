    ...
    $scripts_for_layout = array();
		foreach($this->assets as $asset) {
			...
			if( !empty($this->exceptions) ) {
				$asset_count = count($asset['assets']);
				for($x = 0; $x < $asset_count; $x++) {
					if( in_array($asset['assets'][$x]['script'], $this->exceptions) ) {
						switch($asset['type']) {
							case 'js':
								$scripts_for_layout[] = $this->Javascript->link($asset['assets'][$x]['script']);
								break;
							case 'css':
								$scripts_for_layout[] = $this->Html->css($asset['assets'][$x]['script']);
								break;
							default:
								$scripts_for_layout[] = $asset['assets'][$x]['script'];
						}
						unset($asset['assets'][$x]);
					}
				}
			}
